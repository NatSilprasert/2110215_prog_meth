...
public class TestVenomUnitCard {
    ...
    @Test
    void testAttack() {
        int opponentHealth = oppoUnit1.getHealth();
        int attackDamage1 = venomUnit1.attackUnit(oppoUnit1);
        assertEquals(0, oppoUnit1.getHealth());
        assertEquals(opponentHealth, attackDamage1);

        oppoUnit1.setHealth(opponentHealth);

        int attackDamage2 = venomUnit2.attackUnit(oppoUnit1);
        assertEquals(0, oppoUnit1.getHealth());
        assertEquals(opponentHealth, attackDamage2);

        oppoUnit1.setHealth(opponentHealth);

        int attackDamage3 = venomUnit3.attackUnit(oppoUnit1);
        assertEquals(0, oppoUnit1.getHealth());
        assertEquals(opponentHealth, attackDamage3);
