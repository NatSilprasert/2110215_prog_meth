...
public class TestDeck {
    ...
    void testRemoveCard() {
        try {
            ...
            int oldSize = deck1.getDeckSize();

            Card removed = deck1.getDeckList()[0];
            Card[] beforeRemove = deck1.getDeckList().clone();

            Card result = deck1.removeCard(0);

            assertEquals(removed, result);
            assertEquals(oldSize - 1, deck1.getDeckSize());
            for (int i = 0; i < deck1.getDeckSize(); i++) {
                assertEquals(beforeRemove[i + 1], deck1.getDeckList()[i]);
            }

        } catch (RemoveCardFailedException | InsertCardFailedException e) {
            fail(e.getMessage());
        }

        try {
            if (deck2.getDeckSize() == 0) {
                deck2.insertCard(BuffSpellCard1);
            }
            int oldSize = deck2.getDeckSize();

            Card removed = deck2.getDeckList()[0];
            Card[] beforeRemove = deck2.getDeckList().clone();

            Card result = deck2.removeCard(0);

            assertEquals(removed, result);
            assertEquals(oldSize - 1, deck2.getDeckSize());
            for (int i = 0; i < deck2.getDeckSize(); i++) {
                assertEquals(beforeRemove[i + 1], deck2.getDeckList()[i]);
            }
